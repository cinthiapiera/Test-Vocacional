const SKILLS = {
  ...
  function showFinalResult(){
    // Ordena las habilidades de mayor a menor puntaje y toma las 2 primeras
    const sorted = Object.keys(scores).sort((a,b) => scores[b] - scores[a]);
    const top2 = [sorted[0], sorted[1]];

    const s1 = SKILLS[top2[0]];
    const s2 = SKILLS[top2[1]];

    document.getElementById('res-icon-1').textContent = s1.icon;
    document.getElementById('res-name-1').textContent = s1.name;
    document.getElementById('res-desc-1').textContent = s1.desc;

    document.getElementById('res-icon-2').textContent = s2.icon;
    document.getElementById('res-name-2').textContent = s2.name;
    document.getElementById('res-desc-2').textContent = s2.desc;
